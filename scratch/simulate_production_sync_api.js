...
async function simulateSync() {
  console.log("Simulating server-side /api/videos/sync endpoint...");
  let xmlText = "";
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`YouTube feed fetch returned status ${response.status}. Using fallback.`);
      useFallback = true;
    } else {
      xmlText = await response.text();
    }
  } catch (err) {
    console.warn("Failed to fetch RSS feed:", err);
    useFallback = true;
  }
  ...
  if (useFallback) {
    console.log("Using Fallback Mock Feed...");
    // Fallback logic
  } else {
    console.log("Using Real RSS Feed...");
    const entryMatches = [];
    ...
    console.log(`Parsed ${entryMatches.length} videos from the RSS feed.`);
    ...
    for (const entryXml of entryMatches) {
      ...
      if (publishedTime < CUTOFF_TIMESTAMP) {
        console.log(`- Skipping real video (date constraint): "${title}" (${publishedAtStr})`);
        continue;
      }

      console.log(`- Fetching duration for: "${title}" (${videoId})`);
      let durationSecs = await fetchRealYoutubeDuration(videoId);
      console.log(`  Scraped duration: ${durationSecs} seconds`);

      if (durationSecs === 0) {
        console.log("  No duration scraped. Applying fallback duration of 900s.");
        durationSecs = 900;
      }

      if (durationSecs <= 300) {
        console.log(`  Skipping real video (duration <= 5 mins): "${title}"`);
        continue;
      }
      ...
      syncedVideos.push({
        id: `yt-video-${videoId}`,
        title: title,
        description: extractTagContent(entryXml, "media:description") || "No desc",
  ...
  console.log(`\nSimulation finished. Total synced videos found: ${syncedVideos.length}`);
  for (const fv of syncedVideos) {
    console.log(`* [${fv.id}] "${fv.title}" - Uploaded: ${fv.created_at} - Duration: ${fv.metadata.duration}`);
  }
}
